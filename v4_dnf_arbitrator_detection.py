...
if __name__ == '__main__':
    ...
    if _detection_working != 0:
        ...
        exit(-1)
    # 防止多开,浪费性能
    redis_conn.set(name='v3_detection_working', value='1', ex=10)
    ...
    while True:
        ...
        if len(labels) > 0:
            ...
            for one in labels_detail_list:
                ...
                label_name_list = labels_detail_dict.get(label_name)
                ...
                tmp = {
                    ...
                    'label_box': label_box,
                    'label_box_center': label_box_center,
                }
                label_name_list.append(tmp)

            # list 概率从高到低排序
            for k, v in labels_detail_dict.items():
                labels_detail_dict[k] = sorted(v, key=lambda item: item['label_prob'])

            # 画出坐标
            for k, v in labels_detail_dict.items():
                for tmp_one in v:
                    x, y = tmp_one['label_box_center']
                    arrow_len = 50
                    cv2.line(cv2_mat, pt1=(x, y - arrow_len), pt2=(x, y + arrow_len), color=(0, 0, 255), thickness=3)
